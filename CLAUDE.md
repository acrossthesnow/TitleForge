...
- **TV queries are prefix-first with a retry ladder.** `normalize.title_prefix` (folders) and `classify.series_prefix_from_stem` (episode filenames) take the text **before** the first junk boundary (bracketed year, `S01`/`Season N`/`Complete`, resolution/source/codec token) — never the residue left after deleting junk tokens, which strands separators and episode titles in the query. On zero TMDB results, `prepare_pack_tv_resolve` and `resolve_episode` walk fallback rungs (pack: consensus filename prefix across members, then no year filter; episode: the file's own stem prefix, then first ` - ` segments), logging each fall-through via `_user_notice`. A fallback-rung match caps confidence at `medium` so it surfaces in Phase 1.5, and an episode matched via a *stem-derived* rung never writes `series_by_root` — that guard keeps mixed-series folders (crossover collections) from stamping file #1's show onto its siblings. Locked by `tests/test_prefix_ladder.py`.
- **Filename census outranks folder names.** `_prefix_census` tallies member-filename series prefixes per folder. Disagreeing prefixes (a crossover collection) veto pack binding *and* folder-derived queries — each file searches as itself and nothing is cached on the folder. When members agree but the folder name is unrelated (`_name_similarity` < 0.5, e.g. `TRUE Ending (2005)` holding JLU episodes), the consensus query searches **before** the folder name, so an unrelated folder name that happens to match something on TMDB can't win; a garbage query that *returns results* is invisible to a zero-results retry ladder, which is why ordering matters. Pack members whose own filename prefix contradicts the bound series (< 0.5) reroute to per-file resolution instead of being stamped with the pack id. Leading enumerators — numbered (`1. `) **and lettered** (`a. `–`f. `, the `a. Season 1 (1999)` → query `"a"` → Q&A bug) — are stripped via `normalize.strip_leading_enum` in every query-building path.
- **Destination conflicts surface in Phase 1.5, not just at Proceed.** `_flag_destination_conflicts` (end of `build_plan`) finds entries sharing one destination (same episode ripped into a pack *and* a crossovers folder) or whose destination already exists on disk, pulls each one out of its entity grouping into its own LOW row (`entity_key` repointed to the file) with a "duplicate destination — also from …" reason, and prints a stderr notice per collision at plan time. Phase 2's `_check_duplicate_dests` Proceed guard remains the backstop and now enumerates every group. A file already sitting at its own destination is not a conflict (`samefile` check).

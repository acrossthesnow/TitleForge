...
from titleforge.classify import guess_kind, parse_sxe, series_query_string
...
from titleforge.plex_paths import build_episode_dest, build_movie_dest, parse_tmdb_tag_from_path
...
def resolve_ambiguous_dual(
    ...
    if not candidates:
        ...
        candidates = _gather_dual_candidates(
            tmdb,
            CleanedQuery(title=mq.strip(), year=cleaned.year, raw_stem=cleaned.raw_stem, stripped_year_note=cleaned.stripped_year_note),
            manual_query=mq.strip(),
        )
    if not candidates:
